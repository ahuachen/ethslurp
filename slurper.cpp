...
const CFieldData *findField(const SFString& fieldName)
{
	...
	static CFieldData builtIns[]=
	{
		CFieldData("FIELD_STR", "date",  "date", 0, 0, 0, 0),
		CFieldData("FIELD_STR", "ether", "ether", 0, 0, 0, 0),
	};
	SFInt32 nBuiltIns=sizeof(builtIns)/sizeof(CFieldData);
	for (int i=0;i<nBuiltIns;i++)
	{
		if (fieldName == builtIns[i].getFieldName())
			return &builtIns[i];
	}
